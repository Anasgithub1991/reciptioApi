...
// api between
// router.get('/approvaldaysbetween/:date1', async (req, res) => {
//     const getPaylod = verifyToken(req)
//     if (getPaylod.verify == false) return res.status(400).json(notifyMessage(false, 'Authentication error', '', getPaylod.msg));
//     try {
//         // let requesst = await Requesst.byApproveWithID(req.params.id)
//         let requesst = await Requesst.byCreatedApprovBetweenDate(req.params.date1,req.params.date1,getPaylod.msg);
//         res.status(200).json(notifyMessage(true, "Read succefuly", requesst.data, null));
//     }
//     catch (error) {
//         console.log("error", error)
//         res.status(400).json(error)
//     }
// })
router.get('/approvaldaysbetween/:date1/:date2', async (req, res) => {
    const getPaylod = verifyToken(req)
    if (getPaylod.verify == false) return res.status(400).json(notifyMessage(false, 'Authentication error', '', getPaylod.msg));
    try {
        // let requesst = await Requesst.all()
        let requesst = await Requesst.byCreatedApprovBetweenDate(req.params.date1,req.params.date2,getPaylod.msg);
        res.status(200).json(requesst)
    }
    catch (error) {
        res.status(400).json(error)
